"""
Metrik-4 — S-1 Class-Diagram Similarity Metric (3-field view).
...
A ``MetricProtocol``-conforming adapter (``DISSMetricExtended1``) and a
...
def metric(model1: ParsedModel, model2: ParsedModel) -> MetricResult:
    ...
    6. Project the S-1 sub-scores onto the 3-field ``MetricResult`` schema:
           class_score       <- s1.semantic
           attribute_score   <- s1.propSim
           association_score <- s1.relSim
    ...
    See Also
    --------
    MetricResult, validate_metric_result, DISSMetricExtended1, get_metric
    """
    ...
    result: MetricResult = {
        "class_score":      _clamp(s1.semantic),
        "attribute_score":  _clamp(s1.propSim),
        "association_score": _clamp(s1.relSim),
    }
