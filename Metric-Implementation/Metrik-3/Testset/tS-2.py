#!/usr/bin/env python3
"""
tS-2.py — Compare Metrik-3 metric implementations on the full S1 dataset.
...
def _load_metric(impl_dir: Path):
    ...
    sys.path.insert(0, str(impl_dir.parent))          # Metrik-3/
...
IMPL_KIMI_EXTENDED = D_METRIK_BASE / "Implementation-kimi-extended"

metric_kimi          = _load_metric(IMPL_KIMI)
metric_kimi_extended = _load_metric(IMPL_KIMI_EXTENDED)
...
# ------------------------------------------------------------------
def main():
    # Dataset lives in the project Dataset/ directory.
    dataset_path = D_METRIK_BASE.parent / "Developing-DISS-Metric" / "Dataset" / "combined-data.json"
    if not dataset_path.exists():
        # Fallback: look inside Metrik-3/Dataset/
    ...
    for model_name, model_data in data["models"].items():
        ...
        for setting, gen_uml in generated.items():
            ...
            try:
                result_kimi_extended = metric_kimi_extended(ref_parsed, gen_parsed)
            except Exception as exc:
                errors.append(f"ERROR-METRIC-KIMI-EXTENDED  {label}: {exc}")
                continue

            # Validate EditInformations
            valid_kimi          = isValidEditInformations(result_kimi)
            valid_kimi_extended = isValidEditInformations(result_kimi_extended)
            if not valid_kimi:
                invalid_ei += 1
                errors.append(f"INVAL-EI-KIMI  {label}")
            if not valid_kimi_extended:
                invalid_ei += 1
                errors.append(f"INVAL-EI-KIMI-EXTENDED  {label}")
            if valid_kimi and valid_kimi_extended:
                both_valid += 1

            # Compare total_scaled_distance (tolerance ±0.1)
            if distance_match(result_kimi.total_scaled_distance, result_kimi_extended.total_scaled_distance):
                dist_match += 1
            else:
                dist_diff += 1
                print(
                    ...
                    f"kimi_extended={result_kimi_extended.total_scaled_distance:.4f}"
                )

            # Compare operation counts (tolerance ±1)
            if count_match(len(result_kimi.operations), len(result_kimi_extended.operations)):
                ops_match += 1
            else:
                ops_diff += 1
                print(
                    ...
                    f"kimi_extended={len(result_kimi_extended.operations)}"
                )
