...
IMPL_GML = _D4 / "Implementation-glm"
...
def _run_metric(inst, stud, metric_fn):
    """Run metric and return scalar quality q = mean(scores)."""
    result = metric_fn(inst, stud)

    # Accept both S2-style dict and S1-style EditInformations.
    if hasattr(result, "total_scaled_distance"):
        # S1 metric: single scalar value.
        q = result.total_scaled_distance
        assert isinstance(q, (int, float)), "total_scaled_distance must be numeric"
        assert 0.0 <= q <= 1.0, f"total_scaled_distance={q} out of range [0.0, 1.0]"
        result = {
            "class_score": float(q),
            "attribute_score": float(q),
            "association_score": float(q),
        }
    else:
        # S2 metric: dict with three scores.
        assert isinstance(result, dict), "metric must return a dict or EditInformations"
        dims = ["class_score", "attribute_score", "association_score"]
        for k in dims:
            assert k in result, f"missing key {k}"
            assert isinstance(result[k], (int, float)), f"{k} must be numeric, got {type(result[k])}"
    ...
    return _mean([result[k] for k in ["class_score", "attribute_score", "association_score"]]), result
