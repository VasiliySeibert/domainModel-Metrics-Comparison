# Metrik-5 — S-1 Pipeline (semantic + structural) — Projection v2
...
attribute / association scoring scheme (projection v2 — reuses the UCG
intra/inter-cluster split from Metrik-3).

> Shares its source paper with **Metrik-4** — both implement two different
...
- **Intra-cluster structural attribute matching:** for each semantically
  matched class pair, attribute lists are compared via the UCG intra-cluster
  edit distance (`intraSim`).
- **Inter-cluster structural association matching:** the inter-class
  topology is compared via the UCG inter-cluster edit distance
  (`interSim`).
- **Projection v2:** the three sub-scores are reported as
  `intraSim` / `intraSim` / `interSim` (class / attribute / association),
  reusing the Metrik-3 (UCG) projection shape.
...
```bash
cd Metric-Implementation/Metrik-5
...
|----------:|---------:|----------:|----------------:|
|   0.0738  |  0.1460  |  0.2582   |   **0.1593**    |

**Best overall MAD of the family.** Uses NLTK + WordNet; runs in ~17 minutes
on the full 39-pair test set with multiprocessing.
