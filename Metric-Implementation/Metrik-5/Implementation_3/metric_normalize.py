...
from Implementation_3.metric_models import AttributeInfo, ClassInfo, Edge, Diagram
    ...
    for cls in model.classes:
        attrs = [
            AttributeInfo(
                ...
                modifier="const" if a.is_constant else "",
            )
            for a in cls.attributes
        ]
        classes.append(ClassInfo(name=cls.name, is_abstract=cls.is_abstract, attributes=attrs))
        for enum in cls.nested_enums:
            classes.append(enumToClass(enum))
    for enum in model.enums:
        classes.append(enumToClass(enum))
    for name in model.implicit_classes:
        classes.append(ClassInfo(name=name, is_abstract=False, attributes=[]))

    associations: List[Edge] = []
    dependencies: List[Edge] = []
    generalizations: List[Edge] = []
    ...
    for rel in model.relationships:
        ...
        for edge, kind in edges:
            if kind == "association":
                ...
                generalizations.append(edge)

    # Deduplicate edges within each list to satisfy isValidDiagram
    associations = _dedup_edges(associations)
    dependencies = _dedup_edges(dependencies)
    generalizations = _dedup_edges(generalizations)

    # Deduplicate classes (an enum and an implicit class may share a name)
    seen_names = set()
    unique_classes = []
    for c in classes:
...
def _dedup_edges(edges: list) -> list:
    """Remove duplicate edges (same source, target, ownership, name)."""
    seen = set()
    result = []
    for e in edges:
...
def enumToClass(enum: ParsedEnum) -> ClassInfo:
    ...
    * Because the returned ``ClassInfo`` is not abstract, it will be treated
    ...
    """
    attrs = [AttributeInfo(name=v, type="enum_value", modifier="const") for v in enum.values]
    return ClassInfo(name=enum.name, is_abstract=False, attributes=attrs)
...
def toEdges(rel: ParsedRelationship) -> List[Tuple[Edge, str]]:
    ...
    * **Association-class expansion** – The parser stores the two classes
    ...
    if t == RelationshipType.INHERITANCE:
        return [(Edge(source=rel.source, target=rel.target, name=name, ownership="none"), "generalization")]
    if t == RelationshipType.DEPENDENCY:
        return [(Edge(source=rel.source, target=rel.target, name=name, ownership="none"), "dependency")]
    if t == RelationshipType.COMPOSITION:
        return [(Edge(source=rel.source, target=rel.target, name=name, ownership="composition"), "association")]
    if t == RelationshipType.AGGREGATION:
        return [(Edge(source=rel.source, target=rel.target, name=name, ownership="aggregation"), "association")]
    if t in (RelationshipType.ASSOCIATION, RelationshipType.DIRECTED_ASSOCIATION):
        return [(Edge(source=rel.source, target=rel.target, name=name, ownership="none"), "association")]
    if t == RelationshipType.ASSOCIATION_CLASS:
        x, y = rel.association_members
        z = rel.target
        return [
            (Edge(source=x, target=z, name=name, ownership="none"), "association"),
            (Edge(source=y, target=z, name=name, ownership="none"), "association"),
        ]
    return []
