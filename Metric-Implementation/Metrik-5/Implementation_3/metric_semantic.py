...
sys.path.insert(0, str(_IMPL_DIR.parent))            # = Metrik-5/
sys.path.insert(0, str(_IMPL_DIR.parent / "Testset"))  # = Metrik-5/Testset

from Implementation_3.metric_models import Diagram, SemanticScores
from Testset.metric_invariants import isValidDiagram, isValidSimilarity
from metric_primitives import greedyOptimalSum, CosineSim
...
def propSim(d1: Diagram, d2: Diagram) -> float:
    ...
    * **Complexity** — Building the matrix costs ``O(|C1| * |C2|)`` calls to
    ...
    M = [[_cSim(a, b) for b in C2] for a in C1]
    bestSum = greedyOptimalSum(M)
    return (2.0 * bestSum) / (len(C1) + len(C2))


def _sim(x: str, y: str) -> float:
    return 1.0 if x.lower() == y.lower() else 0.0


def _daSim(a1, a2) -> float:
    return (
        0.1 * _sim(a1.modifier, a2.modifier)
        + 0.7 * CosineSim(a1.name, a2.name)
        + 0.2 * _sim(a1.type, a2.type)
    )


def _aSim(A1, A2) -> float:
    if len(A1) == 0 and len(A2) == 0:
        return 1.0
    if len(A1) == 0 or len(A2) == 0:
        return 0.0
    M = [[_daSim(a, b) for b in A2] for a in A1]
    bestSum = greedyOptimalSum(M)
    return (2.0 * bestSum) / (len(A1) + len(A2))


def _cSim(c1, c2) -> float:
    cnSim = CosineSim(c1.name, c2.name)
    aSim = _aSim(c1.attributes, c2.attributes)
    return 0.571 * cnSim + 0.429 * aSim
...
def _raSim(RA1, RA2) -> float:
    if len(RA1) == 0 and len(RA2) == 0:
        return 1.0
    if len(RA1) == 0 or len(RA2) == 0:
        return 0.0
    M = [[_draSim(a, b) for b in RA2] for a in RA1]
    bestSum = greedyOptimalSum(M)
    return (2.0 * bestSum) / (len(RA1) + len(RA2))


def _draSim(ra1, ra2) -> float:
    return (
        0.3 * CosineSim(ra1.source, ra2.source)
        + 0.2 * CosineSim(ra1.name, ra2.name)
        + 0.2 * (1.0 if ra1.ownership == ra2.ownership else 0.0)
        + 0.3 * CosineSim(ra1.target, ra2.target)
    )


def _rdSim(RD1, RD2) -> float:
    if len(RD1) == 0 and len(RD2) == 0:
        return 1.0
    if len(RD1) == 0 or len(RD2) == 0:
        return 0.0
    M = [[_drdSim(a, b) for b in RD2] for a in RD1]
    bestSum = greedyOptimalSum(M)
    return (2.0 * bestSum) / (len(RD1) + len(RD2))


def _drdSim(rd1, rd2) -> float:
    return (
        CosineSim(rd1.source, rd2.source)
        + CosineSim(rd1.target, rd2.target)
    ) / 2.0


def _rgSim(RG1, RG2) -> float:
    if len(RG1) == 0 and len(RG2) == 0:
        return 1.0
    if len(RG1) == 0 or len(RG2) == 0:
        return 0.0
    M = [[_drgSim(a, b) for b in RG2] for a in RG1]
    bestSum = greedyOptimalSum(M)
    return (2.0 * bestSum) / (len(RG1) + len(RG2))


def _drgSim(rg1, rg2) -> float:
    return (
        CosineSim(rg1.source, rg2.source)
        + CosineSim(rg1.target, rg2.target)
    ) / 2.0
