...
def createGraph(model) -> Graph:
    ...
    """
    vertices = [
        GraphVertex(vertex_id=f"v_{c.name}", element=c)
        for c in model.classes
    ]

    valid_class_names = {c.name for c in model.classes}
    groups: dict[tuple[str, str], list] = {}
    for rel in model.relationships:
        if rel.source in valid_class_names and rel.target in valid_class_names:
            vid1 = f"v_{rel.source}"
            vid2 = f"v_{rel.target}"
            a, b = sorted((vid1, vid2))
            groups.setdefault((a, b), []).append(rel)
