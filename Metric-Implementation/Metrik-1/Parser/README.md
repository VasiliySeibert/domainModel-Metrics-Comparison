...
The parser requires Python 3.7+ with the standard library. No additional external dependencies required.
...
python test_runner.py --references-only

# Continue past failures (don't stop on first error)
python test_runner.py --no-stop

# Test a specific model
python test_runner.py --model LabTracker --setting reference

# Save report to JSON
python test_runner.py --save-report report.json

# List all available models
python test_runner.py --model LabTracker --setting 0shot
```

### Programmatic Usage

```python
from Parser import PlantUMLParser, ParserTestRunner
...
For manual inspection of parsing results (from the Parser directory):
...
python qualitative_test.py 0

# Negative indices work (Python-style)
python qualitative_test.py -1  # Last item
```
...
pytest tests/test_parser_dataset.py -v
```
